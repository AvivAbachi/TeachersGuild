...
module.exports = function (webpackEnv) {
  ...
  const isEnvProductionProfile = isEnvProduction && process
    .argv
    .includes('--profile');

  // We will provide `paths.publicUrlOrPath` to our app as %PUBLIC_URL% in
  // `index.html` and `process.env.PUBLIC_URL` in JavaScript. Omit trailing slash
  // as %PUBLIC_URL%/xyz looks better than %PUBLIC_URL%xyz. Get environment
  // variables to inject into our app.
  ...
  const getStyleLoaders = (cssOptions, preProcessor) => {
    const loaders = [
      ...
      isEnvProduction && {
        ...
        // css is located in `static/css`, use '../../' to locate index.html folder in
        // production `paths.publicUrlOrPath` can be a relative path
        options: paths
          .publicUrlOrPath
          .startsWith('.')
          ? {
        ...
        options: {
          ...
          plugins: () => [
            ...
            // Adds PostCSS Normalize as the reset css with default options, so that it
            // honors browserslist config in package.json which in turn let's users
  ...
  return {
    ...
    output: {
      ...
      // requires a trailing slash, or the file assets will get an incorrect path. We
      ...
      // Prevents conflicts when multiple webpack runtimes (from different apps) are
      // used on the same page.
      jsonpFunction: `webpackJsonp${appPackageJson.name}`,
      // this defaults to 'window', but by setting it to 'this' then module chunks
      // which are built will work in web workers as well.
      globalObject: 'this'
    },
    optimization: {
        ...
        new TerserPlugin({
          terserOptions: {
            parse: {
              ...
              // minification steps that turns valid ecma 5 code into invalid ecma 5 code.
              // This is why the 'compress' and 'output' sections only apply transformations
              // that are ecma 5 safe https://github.com/facebook/create-react-app/pull/4234
              ecma: 8
            },
            compress: {
              ecma: 5,
              ...
              // https://github.com/facebook/create-react-app/issues/5250 Pending further
          ...
          oneOf: [
            ...
            // only compile the standard ES features.
            {
              ...
              options: {
                ...
                // Babel sourcemaps are needed for debugging into node_modules code.  Without
                // the options below, debuggers like VSCode show incorrect code and set
    ...
    plugins: [
      ...
      // network request. https://github.com/facebook/create-react-app/issues/5358
      ...
      // available as %PUBLIC_URL% in index.html, e.g.: <link rel="icon"
      // href="%PUBLIC_URL%/favicon.ico"> It will be an empty string unless you
      // specify "homepage" in `package.json`, in which case it will be the pathname
      // of that URL.
      ...
      // - "files" key: Mapping of all asset filenames to their corresponding   output
      // file so that tools can pick it up without having to parse   `index.html`
      // - "entrypoints" key: Array of files which are included in `index.html`,   can
      // be used to reconstruct the HTML if necessary
      new ManifestPlugin({
      ...
      isEnvProduction && new WorkboxWebpackPlugin.GenerateSW({
        ...
        navigateFallbackBlacklist: [
          ...
          new RegExp('^/_'),
          // Exclude any URLs whose last part seems to be a file extension as they're
          // likely a resource and not a SPA route. URLs containing a "?" character won't
          // be blacklisted as they're likely a route with query params (e.g. auth
          // callbacks).
          new RegExp('/[^/?]+\\.[^/]+$')
        ]
      }),
      ...
      useTypeScript && new ForkTsCheckerWebpackPlugin({
        ...
        reportFiles: [
          '**', '!**/__tests__/**', '!**/?(*.)(spec|test).*', '!**/src/setupProxy.*', '!**/src/setupTests.*'
        ],
        silent: true,
